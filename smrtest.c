...
static int testreader_smr_call(void *x);
...
static int testwriter_arc(void *x);
static int testreader_arc_stats(void *x);
...
static testcase_t test_suite[] = {
    ...
    {"smr_call",   &testreader_smr_call, &testwriter, "smr w/ simple dependent load - not inlined"},
...
static void smrproxy_ref_acquirex(smrproxy_ref_t *ref)
{
    smrproxy_ref_acquire(ref);
}

__attribute__((noinline))
static void smrproxy_ref_releasex(smrproxy_ref_t *ref)
{
...
static int testwriter_arc(void *x)
{
    ...
    arcproxy_t *proxy = env->context.aproxy;
    ...
    while(env->context.active)
    {
        ...
        long long t0 = gettimex(CLOCK_MONOTONIC);
        while (!arcproxy_try_retire(proxy, pdata, &freedata))
        {
            thrd_yield();
        }
        long long t1 = gettimex(CLOCK_MONOTONIC);
...
{
    test_prolog
    arcref_t aref = arcproxy_ref_acquire(env->context.aproxy);
    access_data(env, ppdata, &stats, ndx*10 + ndx2, mod);
    arcproxy_ref_release(env->context.aproxy, aref);
    test_epilog
}
...
static int testreader_smr_call(void * x) {
    test_prolog
    smrproxy_ref_acquirex(ref);
    dependent_load(ppdata);
    smrproxy_ref_releasex(ref);
    test_epilog
}
...
int main(int argc, char **argv) {
    ...
    arcproxy_destroy(env.context.aproxy);
